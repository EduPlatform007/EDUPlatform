document.addEventListener("DOMContentLoaded", function() {
  ...
  // Добавляем проверку наличия функции updateProgress
  if (typeof updateProgress === 'function') {
    updateProgress();
  } else {
    console.log('Функция updateProgress не определена');
  }
  
  // Добавляем проверку наличия функции updateUI
  if (typeof updateUI === 'function') {
    updateUI();
  } else {
    console.log('Функция updateUI не определена');
  }
  
  // Определяем функцию initAccordion для инициализации аккордеонов на странице
  function initAccordion() {
    // Находим все элементы аккордеона на странице
    const accordionItems = document.querySelectorAll('.accordion-item');
    
    if (accordionItems.length === 0) {
      console.log('Элементы аккордеона не найдены');
      return;
    }
    
    console.log('Инициализация аккордеона, найдено элементов:', accordionItems.length);
  ...
  // Вызываем функцию инициализации аккордеона
